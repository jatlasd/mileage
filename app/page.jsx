...
import { addOrder } from "@/models/entry";
...
export default function HomePage() {
  ...
  const addOrderToTrip = async (id) => {
    try {
      await addOrder(id);
      const updatedTrip = await fetch(`/api/entry/${id}`)
       .then((res) => res.json())
       .then((data) => data.trip);
      setActiveTrip(updatedTrip);
    } catch (err) {
      console.error(err);
      alert("Error adding order to trip");
    }
  }
        ...
        <div className="bg-white/[0.07] rounded-xl p-4 mb-6 backdrop-blur-sm border border-white/[0.05]">
          ...
          {activeTrip && (
            <div
              className={`space-y-3 ${
                activeBreak
                  ? "p-3 bg-red-500/10 rounded-lg border border-red-500/20"
                  : ""
              }`}
            >
              <div className="flex items-baseline gap-2">
                ...
                <span className="font-mono text-2xl">
                  {activeTrip.startMileage}
                </span>
              </div>
              <div className="text-sm text-text/60 space-y-0.5">
                <p>
                  Started:{" "}
                  {new Date(activeTrip.startDatetime).toLocaleTimeString()}
                </p>
              </div>
              <div className="flex items-center space-x-4">
                ...
                <button 
                  className="flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg transition-colors disabled:opacity-50 mt-2 bg-red-500/20 hover:bg-red-500/30 text-red-400"
                  onClick={()=>{addOrderToTrip(activeTrip._id)}}
                  >
                  <Plus className="h-5 w-5" /> Log Order
                </button>
              </div>
            </div>
          )}
